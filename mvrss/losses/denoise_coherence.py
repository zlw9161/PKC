...
class DenoiseCoherenceLoss(nn.Module):
    """
    ...
    def __init__(self, global_weight: float = 1.) -> None:
        super(DenoiseCoherenceLoss, self).__init__()
    ...
    def forward(self, rd_input: torch.Tensor,
        ...
        rd_range_avg_probs = torch.mean(rd_softmax, dim=3, keepdim=True)
        ra_range_avg_probs = torch.mean(ra_softmax, dim=3, keepdim=True)
        rd_range_avg_probs = torch.rot90(rd_range_avg_probs, 2, [2, 3])
        rd_range_probs = rd_range_probs - rd_range_avg_probs
        ra_range_probs = ra_range_probs - rd_range_avg_probs
